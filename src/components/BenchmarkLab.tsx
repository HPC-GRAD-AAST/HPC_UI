...
import { useClusters, usePolicies, useTraces, useBenchmarkRun } from "../lib/hooks";
...
export function BenchmarkLab() {
  ...
  const [selectedPolicies, setSelectedPolicies] = useState<string[]>(DEFAULT_POLICIES);
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              <label className="native-label">Policies</label>
              <div className="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-3">
                {(policies ?? []).map((p) => (
                  <label
                    key={p.value}
                    className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm transition-colors ${
                      selectedPolicies.includes(p.value)
                  ...
                  >
                    <input
                      ...
                      checked={selectedPolicies.includes(p.value)}
                      onChange={() => togglePolicy(p.value)}
                    />
                    {p.label}
                  </label>
                ))}
              ...
              {selectedPolicies.length === 0 && (
                <p className="mt-1 text-xs text-destructive">Select at least one policy.</p>
              )}
            </div>
            <div>
