...
function PolicyRowInner({
  ...
  const energyKJ = summaries.reduce((a, r) => a + r.total_energy_joules, 0) / 1000;
  ...
  return (
    <tr className="data-table-row">
      ...
      <td className="data-table-cell">{energyKJ.toFixed(1)} kJ</td>
    </tr>
  );
}
      ...
      <table className="w-full text-sm">
        <thead className="data-table-head">
          <tr>
            ...
            <th className="data-table-th">Total Energy</th>
          </tr>
        </thead>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-4">
        {[
          ...
          { label: "Policies tested", icon: Clock, color: "green", value: String(policies.length) },
          {
            ...
            icon: Zap,
            color: "purple",
            value: String(new Set(doneSims.map((s) => s.cluster_id)).size),
          },
          {
            label: "Data points",
            icon: TrendingUp,
