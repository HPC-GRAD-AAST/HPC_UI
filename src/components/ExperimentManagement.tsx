...
function summaryEntries(summary: ExperimentRunResponse["summary"]): [string, string][] {
  ...
  return Object.entries(summary as Record<string, unknown>)
    .filter(([k]) => k in SUMMARY_LABELS)
    .map(([k, v]) => [SUMMARY_LABELS[k]!, formatMetricValue(v)]);
}
...
function ExperimentConfigDetailDialog({
  ...
  const clusterName = clusters?.find((c) => c.id === cfg?.clusterId)?.name ?? cfg?.clusterId ?? "—";
  const traceName = traces?.find((t) => t.id === cfg?.workloadTraceId)?.name ?? cfg?.workloadTraceId ?? "—";
        ...
        {cfg && (
          <dl className="grid gap-3 text-sm">
            ...
            <div>
              ...
              <dd className="font-mono text-foreground">{cfg.policy.toUpperCase()}</dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd className="text-foreground">{clusterName}</dd>
            </div>
            <div>
              <dt className="text-muted-foreground">Workload trace</dt>
              <dd className="text-foreground">{traceName}</dd>
            </div>
            {cfg.description ? (
              <div>
...
function ExperimentRunDetailDialog({
  ...
  const chartSnapData = useMemo(() => {
    ...
    return sample.map((s) => ({
      ...
      memory: Math.round((s.memory_utilization ?? 0) * 10000) / 100,
                ...
                <TabsContent value="timeline" className="mt-0 space-y-8">
                  {chartSnapData.length === 0 ? (
                    ...
                    <>
                      ...
                      <div className="rounded-xl border border-border bg-card/50 p-4">
                        <h4 className="mb-1 text-sm font-semibold text-foreground">Cluster utilization (CPU &amp; RAM)</h4>
                        <p className="mb-4 text-xs text-muted-foreground">Simulation time (x) vs utilization %</p>
  ...
  const handleCreate = () => {
    ...
    createConfig(
      ...
      },
      {
        onSuccess: () => {
          setShowForm(false);
          setForm({ name: "", cluster_id: "", workload_trace_id: "", policy: "fcfs", seed: 42, description: "", model_id: "" });
        },
      },
    );
  };
            ...
            <div className="border-t border-border pt-6">
              ...
              <div className="mt-4 flex gap-3">
                <Button
                  ...
                  disabled={creating || !form.name || !form.cluster_id || !form.workload_trace_id || (needsModel && !form.model_id)}
                >
